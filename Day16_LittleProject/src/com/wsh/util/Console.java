...
public class Console {
    ...
    public static void logIn() throws DocumentException, UserDataException, IOException {
        Scanner scanner = new Scanner(System.in);
        System.out.println("请输入账号:");
        UserData userData = Source.logIn(scanner.nextLine());
        if (userData != null){ System.out.println("请输入密码:");
            ...
            if (passWord.equals(userData.getPassWord())){ System.out.println("用户"+userData.getName()+"登录成功!");
                while(true) { System.out.println("请选择需要操作的功能:\nⅠ.查询天气\nⅡ.查询手机号归属地\nⅢ.手速游戏\nⅣ.查询手速游戏全国排行榜");
    ...
    public static boolean inspect(String userName ) throws DocumentException {
        Document read = saxReader.read(file);
        Element rootElement = read.getRootElement();
        List<Element> elements = rootElement.elements();
        for (Element element : elements) {
            String text = element.element("userName").getText();
            if (text.equals(userName)){ System.out.println("账号已存在!");
