...
@EnableAspectJAutoProxy(proxyTargetClass = true)// 不管目标类是否实现接口,都想用CGLIB的方式实现动态代理
public class Transaction {
    ...
    @Pointcut("execution(String com.wsh.aop.UserDaoImpl.*(..))")
    ...
    public void begin(JoinPoint joinPoint){
        System.out.println("-------------begin~~连接点:"
                + joinPoint.getSignature().getName() +"  目标类:"+ joinPoint.getTarget().getClass().getName());
    }
    // 当方法成功返回后会执行
    // 若方法抛出异常,则不会执行
    // returning = "getS":指定,方法的返回值要传入到commit的哪个参数中
    @AfterReturning(value = "pointCut()",returning = "getS")
    public void commit(String getS){
        System.out.println("-------------commit");
    }
    // 最终通知
    // 不管切入点是否执行成功(是执行完毕了,还是抛出异常了)
    // 最终通知都会执行
