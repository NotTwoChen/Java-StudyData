...
function changePost(departmentObj,postId){
	...
	var url = "${pageContext.request.contextPath}/post_findByDepId?depId="+depId;
	...
	xmlhttp.onreadystatechange = function(){
        ...
        if(xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			...
			postSelectObject.firstElementChild.remove();
            postSelectObject.innerHTML = "<option value='"+"'>----请--选--择----</option>";

			//获得数据 json，并处理
			//@1 获得普通文本数据
            var textData = xmlhttp.responseText;
            // var number1 = textData.indexOf("(");
            // var number2 = textData.indexOf(")");
            // textData = textData.substring(number1+1,number2);
